...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2_async__ = __webpack_require__("../../../../async/dist/async.js");
...
var AppService = (function () {
    ...
    AppService.prototype.getWallPosts = function (_a) {
        var owner_id = _a.owner_id, user_access_token = _a.user_access_token, count = _a.count, offset = _a.offset;
        var url = ("\n        https://api.vk.com/api.php?\n            oauth=1&\n            method=wall.get&\n            owner_id=" + owner_id + "&\n            offset=" + offset + "&\n            count=" + count + "&\n            filter=all&\n            access_token=" + user_access_token)
            .replace(/ /g, '')
            .replace(/\n/g, '');
        return __WEBPACK_IMPORTED_MODULE_0_fetch_jsonp__(url)
            ...
            var response = _a.response;
            if (response) {
                var available = response[0], posts = response.slice(1);
                return { available: available, posts: posts };
            }
            return null;
        })
            .catch(function (ex) { return console.log('parsing failed', ex); });
    ...
    ;
    AppService.prototype.getAllWallPosts = function (_a) {
        var owner_id = _a.owner_id, user_access_token = _a.user_access_token, count = _a.count, offset = _a.offset;
        var httpRequest = function (count, callback) {
            return this.appService.getWallPosts({
                ...
                .then(function (response) { return callback(response.posts); });
        };
        var apiQuery = function (count, apiQueryCallback) {
            ...
            var queue = __WEBPACK_IMPORTED_MODULE_2_async__["async"].queue(function (offset, callback) {
            ...
            __WEBPACK_IMPORTED_MODULE_2_async__["async"].eachLimit(offsets, maxRPS, function (offset, callback) {
...
var FeedComponent = (function () {
    function FeedComponent(appService, authService, snackBar) {
        ...
        this.counter = 0;
        this.selectedTab = 1;
        this.showTimer = function (timer, counter, TIMEOUT_STEP) {
            var timeBase = (timer - counter * TIMEOUT_STEP) / 1000;
            return timeBase + 'СЕК';
        };
    }
    FeedComponent.prototype.getWallPosts = function (_a) {
        var _this = this;
        var owner_id = _a.owner_id, count = _a.count, user_access_token = _a.user_access_token, _b = _a.posts, posts = _b === void 0 ? [] : _b, _c = _a.offset, offset = _c === void 0 ? 0 : _c, _d = _a.pause, pause = _d === void 0 ? 0 : _d;
        return this.appService.getWallPosts({
            owner_id: owner_id,
            count: count,
            user_access_token: user_access_token,
            ...
            if (response) {
                ...
                _this.postsAvailable = response.available;
                posts = posts.concat(response.posts);
                console.log(offset, count, _this.postsAvailable);
                if (count > 0 && _this.postsAvailable >= offset) {
                    pause += 400;
                    return _this.getWallPosts({ owner_id: owner_id, count: count, user_access_token: user_access_token, posts: posts, offset: offset, pause: pause });
                }
                else {
                    return posts;
                }
        ...
        if (this.authService.loggedIn()) {
            ...
            this.getWallPosts({
                ...
                if (posts) {
                    ...
                    _this.snackBar.open('Не получилось запросить посты', 'ОК');
                }
            });
        }
    };
            ...
            if (!response) {
                ...
                if ('deactivated' in _this.currentUser) {
                    ...
                    _this.appService
                        ...
                        if (groupList.length) {
                            _this.currentUserGroups = groupList;
                        }
                        else {
                            _this.radio = 'other_groups';
